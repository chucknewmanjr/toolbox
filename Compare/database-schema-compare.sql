...
select 
	...
	CONCAT_WS('.', OBJECT_SCHEMA_NAME(parent_object_id), OBJECT_NAME(parent_object_id), COL_NAME(parent_object_id, parent_column_id)),
	'Check Constraint',
	[definition]
...
if OBJECT_ID('tempdb.dbo.##Schema_Snapshot') is not null begin;
	declare @source table (Item nvarchar(500), Parent nvarchar(500), Property_Type sysname, Properties nvarchar(MAX));

	declare @target table (Item nvarchar(500), Parent nvarchar(500), Property_Type sysname, Properties nvarchar(MAX));
...
values
	...
	(15, 'create table ##Schema_Snapshot (Item nvarchar(200), Parent nvarchar(500), Property_Type sysname, Properties nvarchar(MAX));'), 
